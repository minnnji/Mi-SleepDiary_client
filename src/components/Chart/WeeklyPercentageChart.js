...
const WeeklyPercentageChart = (props) => {
  ...
  useEffect(() => {
    ...
    const stackGenerator = d3.stack().keys(keys),
          ...
          maxRange = [0, d3.max(sleepsByType, sleepList => d3.max(sleepList, sequence => sequence[1]))],
          dayShort = d3.map(data, function(d){return(d.day)}).keys();
    ...
    const barByType = bars.append("g")
      .attr("fill", sleepsByType => colors[sleepsByType.key]);
